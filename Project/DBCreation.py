...
# Data for the list of Medicines
...
[23,"Ophthalmic","Vigatex","Eyedrop",1000,"moxifloxacin"],
...
try:
        # open database connection by passing the connection parameters
        dbConnection=mysql.connector.connect(host="localhost",user="root",password="0410",database="project")
        cursor=dbConnection.cursor()
        print("DB connection success")
        # Drop all tables
        cursor.execute("Drop table meds_db")
        cursor.execute("Drop table patientpresc")
        cursor.execute("Drop table patientdetails")

        # create table meds_db 
        ...
        # CSV file creation- for storing the medicine values 
